"""M6: constrained_loss in the REAL default regime -- M5's protocol with the loss switched on.

Compare against M5 (same seeds; identical except `constrained_loss`), which shows the loss's effect at every
checkpoint rule. Runs from a worktree whose library has `constrained_loss` (exp/constrained-loss).

(Original M5 docstring follows.)
...
checkpoint rule can be compared on identical trajectories.
    ...
    python research/m5.py run --name m6 --arms default_cl wk_cl --datasets diabetes insurance \
...
ARMS = {"default": dict(gpt2={}, train_args={}, init={}),
        "default_cl": dict(gpt2={}, train_args={}, init={"constrained_loss": True}),
        "wk_cl": dict(gpt2={"n_embd": 128, "n_head": 4, "n_layer": 3}, train_args={"learning_rate": 3e-4, "warmup_steps": 0.05}, init={"constrained_loss": True}),
...
if __name__ == "__main__":
    ...
    r = sub.add_parser("run"); r.add_argument("--name", required=True); r.add_argument("--arms", nargs="+", default=["default_cl", "wk_cl"])
