...
from .rtf_shared import (
    ...
    _normalize_gpt2_state_dict,
    _validate_get_device,
)
from .rtf_trainer import ResumableTrainer

# Default backbone model IDs
DEFAULT_TABULAR_BACKBONE = "distilgpt2"
DEFAULT_RELATIONAL_ENCODER = "distilgpt2"
DEFAULT_RELATIONAL_DECODER = "distilgpt2"


def _set_num_layers(cfg: PretrainedConfig, n: int) -> None:
    """Set the number of layers in a config, handling different architecture field names."""
    # GPT2-like
...
def _get_max_positions(cfg: PretrainedConfig) -> Optional[int]:
    """Get the maximum position embeddings from a config."""
...
def _set_max_positions(cfg: PretrainedConfig, n: int) -> None:
    """Set the maximum position embeddings in a config."""
    if hasattr(cfg, "n_positions"):
        cfg.n_positions = n
    elif hasattr(cfg, "max_position_embeddings"):
        cfg.max_position_embeddings = n


def _maybe_enable_cross_attention(cfg: PretrainedConfig) -> None:
    """Enable cross-attention for decoder configs that support it."""
...
def _maybe_normalize_parent_state_dict(cfg: PretrainedConfig, sd: dict) -> dict:
    """Normalize state dict only for GPT2-like models."""
    # Only GPT2 uses the `transformer.` prefix pattern we're stripping
    if getattr(cfg, "model_type", None) == "gpt2":
        return _normalize_gpt2_state_dict(sd)
    return sd


class REaLTabFormer2(SharedModelMixin):
    def __init__(
        self,
        model_type: str,
        tabular_config: Optional[PretrainedConfig] = None,
        relational_config: Optional[EncoderDecoderConfig] = None,
        tabular_backbone: str = DEFAULT_TABULAR_BACKBONE,
        relational_encoder_backbone: str = DEFAULT_RELATIONAL_ENCODER,
        relational_decoder_backbone: str = DEFAULT_RELATIONAL_DECODER,
        ...
        **training_args_kwargs,
        ...
        Args:
            ...
            tabular_config: PretrainedConfig instance to customize the causal LM model for tabular data
                generation. If None, uses the model specified by `tabular_backbone` (default: distilgpt2).
            ...
            tabular_backbone: Model ID for the tabular model backbone (default: "distilgpt2").
                Used when `tabular_config` is None.
            relational_encoder_backbone: Model ID for the relational encoder backbone (default: "distilgpt2").
                Used when `relational_config` is None.
            relational_decoder_backbone: Model ID for the relational decoder backbone (default: "distilgpt2").
                Used when `relational_config` is None.
            ...
            epochs: Number of epochs for training the causal LM model. Use a large number of epochs to take advantage of the framework's optimal termination feature for the non-relational tabular data model. Defaults to 1000.
            batch_size: Batch size used for training. Defaults to 32 for tabular models and 8 for
                relational models (matching REaLTabFormer's V1 default). `gradient_accumulation_steps`
                is derived from this as `max(1, round(32 / batch_size))`, so the effective batch size
                stays ~32 regardless of the chosen `batch_size`. An explicit `gradient_accumulation_steps`
                in **training_args_kwargs overrides.
        ...
        # Store backbone model IDs
        self.tabular_backbone = tabular_backbone
        self.relational_encoder_backbone = relational_encoder_backbone
        self.relational_decoder_backbone = relational_decoder_backbone
        ...
        if self.model_type == ModelType.tabular:
            ...
            self.parent_encoder_config_dict = None
            self.parent_encoder_state_dict = None
            ...
            if parent_realtabformer_path is not None:
                ...
                self.parent_encoder_config_dict = parent_config["tabular_config"]
                # Reconstruct config to determine model type for normalization
                mt = self.parent_encoder_config_dict.get("model_type", "gpt2")
                try:
                    from transformers import CONFIG_MAPPING

                    config_class = CONFIG_MAPPING[mt]
                    parent_cfg = config_class.from_dict(self.parent_encoder_config_dict)
                except (KeyError, ImportError):
                    # Fallback: use GPT2Config for backward compatibility
                    from transformers.models.gpt2 import GPT2Config

                    parent_cfg = GPT2Config.from_dict(self.parent_encoder_config_dict)
                raw_sd = torch.load(
                    parent_realtabformer_path / ModelFileName.rtf_model_pt
                )
                self.parent_encoder_state_dict = _maybe_normalize_parent_state_dict(
                    parent_cfg, raw_sd
                )
    ...
    # _invalid_model_type is inherited from SharedModelMixin.
    ...
    def _init_relational(self, relational_config):
        if relational_config is not None:
            ...
            # Default is 12, use 6 for distill-gpt2 as default
            enc = AutoConfig.from_pretrained(self.relational_encoder_backbone)
            dec = AutoConfig.from_pretrained(self.relational_decoder_backbone)

            _set_num_layers(enc, 6)
            _set_num_layers(dec, 6)

            _maybe_enable_cross_attention(dec)

            relational_config = EncoderDecoderConfig.from_encoder_decoder_configs(
                enc, dec
            )

        if self.parent_encoder_config_dict is not None:
            warnings.warn(
            ...
            try:
                ...
                parent_cfg = GPT2Config.from_dict(self.parent_encoder_config_dict)
            relational_config.encoder = parent_cfg

        self.relational_config = relational_config
        self.model = None

    # _extract_column_info, _generate_vocab, _check_model, and
    # _split_train_eval_dataset are inherited from SharedModelMixin.
    ...
    def fit(
        ...
        Args:
            ...
            digit_entropy_weighting: If True, numeric/datetime columns' digit-chunk loss weights
              ...
              and reallocates weight *within* each original column's own chunks only -- it never
              changes a column's total weight relative to other columns. See
              `data_utils.vocab.compute_chunk_significance_weights`.
            digit_entropy_weight_floor: Floor applied to a chunk's raw (pre-renormalization) weight
              so a fully degenerate chunk never gets exactly zero weight -- it can still matter for
              the rare row where it's not degenerate. Only used when `digit_entropy_weighting=True`.
    ...
    # get_full_save_dir and get_experiment_id are inherited from
    # SharedModelMixin -- this used to be an independent reimplementation
    # that got the epoch-vs-experiment_id precedence backwards and raised
    # on a resumed fit()'s periodic checkpoint save. See
    # notes/lab_notebook.md, 2026-09-19.
    ...
    def _set_up_relational_coder_configs(self) -> None:
        ...
        for coder_name in ["encoder", "decoder"]:
            ...
            if coder_name == "decoder":
                cur_pos = _get_max_positions(coder)
                if cur_pos is not None and cur_pos < self.relational_max_length:
                    _set_max_positions(coder, 128 + self.relational_max_length)
            elif coder_name == "encoder":
                cur_pos = _get_max_positions(coder)
                if cur_pos is not None and cur_pos < len(
                    self.vocab[coder_name]["column_token_ids"]
                ):
                    _set_max_positions(
                        coder, 128 + len(self.vocab[coder_name]["column_token_ids"])
                    )

        # This must be set to True for the EncoderDecoderModel to work at least
        # with GPT2 as the decoder.
        _maybe_enable_cross_attention(self.relational_config.decoder)
    ...
    def _fit_relational(
        ...
        if self.parent_encoder_state_dict is not None:
            pretrain_load = self.model.encoder.load_state_dict(
                self.parent_encoder_state_dict, strict=False
            )
    ...
    def _fit_tabular(
        ...
        # Make sure that we have at least the number of
        # columns in the transformed data as positions.
        need_pos = 128 + len(self.vocab["column_token_ids"])
        cur_pos = _get_max_positions(self.tabular_config)
        if cur_pos is not None and cur_pos < len(self.vocab["column_token_ids"]):
            _set_max_positions(self.tabular_config, need_pos)

        self.model = AutoModelForCausalLM.from_config(self.tabular_config)
    ...
    # sample and predict are inherited from SharedModelMixin.
    ...
    def save(
        ...
        for ignore_key in [
            "parent_vocab",
            "parent_encoder_config_dict",
            "parent_encoder_state_dict",
            "parent_col_transform_data",
            "parent_gpt2_config",  # Backward compatibility
            "parent_gpt2_state_dict",  # Backward compatibility
        ]:
            if ignore_key in rtf_attrs:
                rtf_attrs.pop(ignore_key)

        # PretrainedConfig objects are not JSON serializable, let us manually
        # extract the attributes. The to_dict() method includes model_type which
        # is needed for proper reconstruction during loading.
    ...
    def load_from_dir(cls, path: Union[str, Path]):
        ...
        # Helper function to reconstruct config from dict
        def _config_from_dict(d: dict) -> PretrainedConfig:
            # Most HF config dicts include "model_type"
            mt = d.get("model_type")
            if mt is None:
                # fallback: assume GPT2 for backward compatibility
                # Import here to avoid circular dependency
                from transformers.models.gpt2 import GPT2Config

                return GPT2Config.from_dict(d)
            # Use transformers config registry to get the right config class
            try:
                from transformers import CONFIG_MAPPING
        ...
        # special case for the configs.
        for k, v in rtf_attrs.items():
            if k == "gpt_config":
                # Back-compatibility for saved models
                # before the support for relational data
                # was implemented.
                v = _config_from_dict(v)
                k = "tabular_config"
        ...
        # Load model weights
        if realtf.model_type == ModelType.tabular:
            realtf.model = AutoModelForCausalLM.from_config(realtf.tabular_config)
        elif realtf.model_type == ModelType.relational:
